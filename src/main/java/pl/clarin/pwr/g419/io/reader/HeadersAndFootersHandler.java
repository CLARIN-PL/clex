...
public class HeadersAndFootersHandler {
  ...
  public void findAndExtractHeadersAndFooters(HocrDocument document) {
    ...
    // diagnostyka
    log.debug("Doc:" + document.getId() + "- Printing headers -");
    document.getDocContextInfo().getHeaders().stream().forEach(h -> log.debug(" " + h));
    log.debug("Doc:" + document.getId() + "- Printing footers -");
  ...
  private boolean areLinesTheSame(HocrLine firstHocrLine, HocrLine secondHocrLine) {
    ...
    if (secondHocrLine == null) {
      ...
      if (!firstHocrLine.getText().trim().equalsIgnoreCase(secondHocrLine.getText().trim())) {
        ...
        // Sprawdzanie  jeszcze czy nie różni się tylko numerem strony (sporo przypadków) - branch. 68
        result = false;
      } else {
        // czy to nie jest zupełnie inna wysokości linii ale z tym samym tekstem ... ?
        result = firstHocrLine.getHeight() - secondHocrLine.getHeight() <= SIGNIFICANT_DIFF_IN_RANGE_HEIGHT;
      }
    }
    return result;
  }
